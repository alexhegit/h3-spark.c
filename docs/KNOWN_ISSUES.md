...
| ID | Topic | Status |
|----|-------|--------|
| D | Perf + CUDA `--profile` | **v0.2.2** — fox-fast 15.6 s e2e / 8.17 s denoise; 15 s 17 min 56 s; 15 s+TR **11 min 14 s**; 15 s+`--sol-attn` **11 min 35 s** (PSNR 19.2). Remaining: fox-fast INT8 linear; 15 s long-N SDPA. [`PERF_BASELINE.md`](PERF_BASELINE.md) |
